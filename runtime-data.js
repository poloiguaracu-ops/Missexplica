...
(function(){
  ...
  async function studentCourses(){const u=await requireUser();const {data,error}=await sb().from('enrollments').select('id,status,courses(id,title,description,published,modules(id,title,position,lessons(id,title,description,position,published,video_url,materials(id,title,url,type))))').eq('student_id',u.id).in('status',['active','completed']);if(error) throw error;return (data||[]).map(e=>({...e,course:e.courses}));}
