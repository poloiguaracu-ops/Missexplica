...
(function(){
 ...
 async function waitForSupabase(){if(window.missexplicaSupabase)return window.missexplicaSupabase;if(window.missexplicaSupabaseReady)return await window.missexplicaSupabaseReady;throw new Error('Serviço de autenticação ainda não foi configurado.')}
 function mount(){
  ...
  form.addEventListener('submit',async e=>{
   ...
   if(!valid||passwordValue.length<8){message('Informe seu CPF ou RU e uma senha com pelo menos 8 caracteres.');return}
