...
(function(){
  ...
  function messagesPage(){
    ...
    D.querySelectorAll('.mx38-read').forEach(b=>b.onclick=()=>{let a=getMessages(),i=a.findIndex(x=>x.id==b.dataset.id);if(i>=0){a[i].read=true;saveMessages(a);messagesPage();}});
    D.querySelectorAll('.mx38-delete').forEach(b=>b.onclick=()=>{saveMessages(getMessages().filter(x=>x.id!=b.dataset.id));messagesPage();});
    const all=D.getElementById('mx38ClearRead');if(all)all.onclick=()=>{saveMessages(getMessages().map(x=>({...x,read:true})));messagesPage()};
    const send=D.getElementById('mx38Send');if(send)send.onclick=()=>{const to=D.getElementById('mx38To').value.trim(),subject=D.getElementById('mx38Subject').value.trim(),body=D.getElementById('mx38Body').value.trim();if(!to||!subject||!body){toast('Preencha destinatário, assunto e mensagem.');return}const a=getMessages();a.unshift({id:Date.now(),from:s.user||'Usuário',to,subject,body,date:new Date().toLocaleDateString('pt-BR'),read:true});saveMessages(a);toast('Mensagem salva neste dispositivo.');messagesPage()};
    return true;
  }
