...
Deno.serve(async req=>{
 ...
 try{
  ...
  const {data:profile,error:profileError}=await admin.from('profiles').select('id,full_name,role,active,cpf,ru').eq(isRu?'ru':'cpf',subjectValue).eq('role','student').eq('active',true).maybeSingle();
  if(profileError||!profile)throw new Error('CPF/RU ou senha inválidos.');
  const {data:authUser,error:authUserError}=await admin.auth.admin.getUserById(profile.id);
  const email=authUser?.user?.email;
  if(authUserError||!email)throw new Error('Conta de acesso não configurada.');
  const authClient=createClient(base,anon);
  const {data:authData,error:authError}=await authClient.auth.signInWithPassword({email,password:passwordValue});
