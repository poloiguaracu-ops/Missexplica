...
(function(){
  ...
  async function renderRealCourses(){
    ...
    container.innerHTML='<div class="panel"><div class="loading-state">Carregando seus cursos...</div></div>';
    try{
      const courses=await window.loadStudentCourses();
      if(!courses)return;
      container.innerHTML=`<div class="welcome"><div><div class="section-tag">MEUS CURSOS</div><h1>Continue aprendendo</h1><p>Somente cursos liberados para sua matrícula aparecem aqui.</p></div></div><div class="panel">${realCoursesMarkup(courses)}</div>`;
      ...
      container.innerHTML='<div class="panel"><div class="empty"><strong>Não foi possível carregar seus cursos.</strong><br><span>Verifique sua conexão e tente novamente.</span><br><button class="action" id="retryStudentCourses">Tentar novamente</button></div></div>';
  ...
  function openRealCourse(course){
    ...
    document.getElementById('completeRealLesson')?.addEventListener('click',async function(){
      ...
      const {error}=await window.missExplicaSupabase.rpc('student_complete_lesson',{target_lesson:firstLesson.id});
  ...
  if(typeof originalRender==='function'){
    window.render=function(){
      ...
      if(role==='student'&&title==='Meus cursos')setTimeout(renderRealCourses,0);
    };
  }
  window.missExplicaRenderRealCourses=renderRealCourses;
  document.addEventListener('click',function(e){
    if(e.target.closest?.('[data-page="courses"]'))setTimeout(renderRealCourses,0);
  });
})();
